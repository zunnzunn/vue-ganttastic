...
export default function useBarDragLimit() {
  ...
  const countGapDistanceToNextImmobileBar = (
    ...
    if (side === "left") {
      while (nextBar) {
        const currentBarElem = document.getElementById(
          currentBar.ganttBarConfig.id
        ) as HTMLElement
        const nextBarElem = document.getElementById(
          nextBar.ganttBarConfig.id
        ) as HTMLElement
        const nextBarOffsetRight =
          nextBarElem.offsetLeft + nextBarElem.offsetWidth
    ...
    if (side === "right") {
      while (nextBar) {
        ...
        const currentBarOffsetRight =
          currentBarElem.offsetLeft + currentBarElem.offsetWidth
  ...
  const getNextGanttBar = (bar: GanttBarObject, side: "left" | "right") => {
    ...
    if (side === "left") {
      allBarsLeftOrRight = allBarsInRow.filter((otherBar) => {
        const otherBarElem = document.getElementById(
    ...
    if (allBarsLeftOrRight.length > 0) {
      return allBarsLeftOrRight.reduce((bar1, bar2) => {
        const bar1Elem = document.getElementById(
          bar1.ganttBarConfig.id
        ) as HTMLElement
        const bar2Elem = document.getElementById(
